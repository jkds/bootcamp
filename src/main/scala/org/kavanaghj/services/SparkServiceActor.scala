...
trait SparkService extends HttpService with StatUtils with Logging {
  ...
  val route = path("numOfReviews") {
    ...
    get {
      logInfo("Received request to retrieve the most popular product")
  ...
  private def getProductRatings(ctx : CassandraSQLContext) : scala.collection.mutable.Map[String,(Int,Int)] = {
    ...
    logInfo(">>>> Retrieved list of products/scores. Processing into a map")
    ...
    logInfo(">>>> Finished mapping of product and negative/positive scores")
    map
  }
